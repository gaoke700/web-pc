...
var gulpSeajsCombine = require('gulp-seajs-combine');
...
gulp.task('assets-js-component', function () {
    return gulp.src(['src/js/component/component-main.js'])
        .pipe(gulpSeajsCombine())
        .pipe(gulpBabel({ presets: ['es2015'] }))
        .pipe(gulp.dest('assets/js/component'));
});

gulp.task('assets-js-component-loader', function () {
    return gulp.src(['src/js/component/component-loader.js'])
        .pipe(gulpBabel({ presets: ['es2015'] }))
        .pipe(gulp.dest('assets/js/component'));
});

gulp.task('assets-js-all', ['assets-js-component-loader', 'assets-js-component'], function(){
    return gulp.src(['src/js/**/*.js', '!src/js/base/**/*.js', '!src/js/component/*.js'])
        .pipe(gulpBabel({ presets: ['es2015'] }))
        .pipe(gulp.dest('assets/js'));
});
...
gulp.task('dev-init-config', ['dev-rev'], function(){
    var manifest = Fs.readFileSync('assets/rev-manifest.json','utf-8'),
        manifestJson = JSON.parse(manifest),
        config = { base: staticPath, alias: {} };
    for(var i in manifestJson){
        config.alias[(i.replace(/\.js$/g, ""))] = manifestJson[i];
    }

    Fs.writeFileSync('assets/js/config.js','seajs.config(' + JSON.stringify(config,null,4) + ');');
});
...
gulp.task('dev-rev-collector-html', ['dev-rev', 'dev-inline-source-html', 'dev-init-config'], function() {
    var manifest = Fs.readFileSync('assets/rev-manifest.json','utf-8'),
        manifestJson = JSON.parse(manifest),
        config = { base: staticPath, alias: {} };
    for(var i in manifestJson){
    ...
    var seajsConfig = 'seajs.config(' + JSON.stringify(config,null,4) + ');';

    var reg = /<\!--#import\s+(.+)\s+#source-->/gim;

    return gulp.src(['assets/rev-manifest.json', 'assets/view/**/*.html', 'assets/view-lua/**/*.html'], { base: './assets/' })
        .pipe(gulpRevCollector({ doMainName: staticPath }))
        .pipe(gulpReplace(reg, function(){
